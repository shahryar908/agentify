...
def render_latex_pdf(latex_content: str) -> str:
    ...
    try:
        ...
        # Step3: Setup filenames (only tex input, tectonic will auto create PDF)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        tex_filename = f"paper_{timestamp}.tex"
        tex_file = output_dir / tex_filename

        # Step4: Write LaTeX content to .tex file
        tex_file.write_text(latex_content)

        # Step5: Run tectonic to generate PDF
        ...
        # Step6: tectonic outputs PDF with same basename as .tex
